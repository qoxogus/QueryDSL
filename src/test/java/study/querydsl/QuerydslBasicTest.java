...
//                .where(m.username.eq("member1")) //파라미터 바인딩 처리
    ...
    public void findUserDto() {
        ...
        List<UserDto> result = queryFactory
                .select(Projections.fields(UserDto.class, //fields -> 필드 명이 맞아야함(일치해야함, setter도 마찬가지)
                        member.username.as("name"), //as로 이름을 지정해주면 값이 들어간다.
        ...
        for (UserDto userDto : result) {
            System.out.println("userDto = " + userDto); //오류가 나진않지만 name이 매칭되는게 없어서 null로 들어간다.
        }
    }
    ...
    } //이 방식들은 생성자들이 있어야한다 (select none, all cinstructor)
...
//        return member.username.eq(usernameCond);
    ...
    public void bulkUpdate() {
        ...
        long count = queryFactory //영향받은 행 개수
                .update(member)
                .set(member.username, "비회원")
                .where(member.age.lt(28)) //나이가 28살 미만이면
                .execute();
        ...
        //member3 = 30 -> DB member3
        //member4 = 40 -> DB member4
        //하지만 영속성 컨텍스트에는 실행되기 전처럼 남아있다!!!!

        em.flush(); //DB와 영속성 컨텍스트를 맞추고
        em.clear(); // 영속성 컨텍스트를 초기화 해버리자
    ...
    public void bulkDelete() {
        long count = queryFactory
                .delete(member)
                .where(member.age.gt(18)) //18살 이상의 회원을 지우는 쿼리
                .execute();
    }
